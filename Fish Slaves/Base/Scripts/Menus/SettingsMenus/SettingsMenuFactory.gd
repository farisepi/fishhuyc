...
# Путь к текстуре для выпадающего меню (Factory)
...
func _apply_texture_to_option_button(option_btn: OptionButton, texture: Texture2D) -> void:
	...
	# 2. УБИРАЕМ СТАНДАРТНЫЕ РАДИОКНОПКИ ЧЕРЕЗ СТИЛЬ
	...
	# 4. ОТКЛЮЧАЕМ РАДИОКНОПКИ И УСТАНАВЛИВАЕМ ИКОНКИ
	...
	popup.add_theme_constant_override("item_padding", 25)  # Отступ слева
...
func _on_back_pressed() -> void:
	...
	current_tree.change_scene_to_file("res://Fish Slaves/Base/Scenes/Menus/MainMenus/MainMenuFactory.tscn")
